...
from models.questions.rearrange import CollegeRearrange as Rearrange, Item, Image
from utils.jwt import verify_access_token
from utils.response import response
from datetime import datetime

rearrange_bp = Blueprint("college_rearrange_bp", __name__)
...
def add_rearrange():
    try:
        ...
        rearr = Rearrange(
            ...
            # preserve optional denorm fields if your model uses them
            # created_at=getattr(data, "created_at", None) or None,
            # updated_at=getattr(data, "updated_at", None) or None,
        )
...
def get_rearranges():
    try:
        ...
        # additionally restrict by admin's college_id if available; if admin has no college_id, return empty set
        admin_college_id = _get_admin_college_id()
        if admin_college_id is None:
            total = 0
...
def get_rearrange(rearrange_id):
    try:
        ...
        # enforce college ownership
        forbid = _ensure_same_college_or_forbid(getattr(rearr, "college_id", None))
        if forbid:
            return forbid
